...
public class Board : MonoBehaviour
{
    ...
    private int matchesTileCount = 2;
    ...
    {
        int selectedTileCount = 2;

        if (_selection.Contains(tile) == false)
            _selection.Add(tile);

        if (_selection.Count < selectedTileCount)
            return;
    ...
    private bool CanPop()
    {
        for (var y = 0; y < Height; y++)
        {
            ...
            {
                if (Tiles[x, y].GetConnectedTiles().Skip(1).Count() >= matchesTileCount)
                    return true;
            }
        }
    ...
    private async void Pop()
    {
        for (var y = 0; y < Height; y++)
        {
            for (var x = 0; x < Width; x++)
            {
                ...
                if (connectedTiles.Skip(1).Count() < matchesTileCount)
                    continue;
